...
public class MovieAnalyzer {
  ...
  public MovieAnalyzer(String dataset_path) {
    try (BufferedReader csvReader = new BufferedReader(new FileReader(dataset_path, StandardCharsets.UTF_8))) {
      ...
      while ((data = csvReader.readLine()) != null) {
        ...
        String[] filteredLine = Stream.of(line)
  ...
  public Map<Integer, Integer> getMovieCountByYear() {
    return dataset.stream()
        ...
        .collect(Collectors.groupingBy(s -> Integer.parseInt(s[2]),
  ...
  public Map<String, Integer> getMovieCountByGenre() {
    return Arrays.stream(dataset.stream()
  ...
  public Map<List<String>, Integer> getCoStarCount() {
    return dataset.stream()
        .flatMap(s -> Stream.of(sortedCoStar(s[10], s[11]), sortedCoStar(s[10], s[12]),
            sortedCoStar(s[10], s[13]), sortedCoStar(s[11], s[12]),
            sortedCoStar(s[11], s[13]), sortedCoStar(s[12], s[13])))
        .collect(Collectors.groupingBy(Function.identity(),
            Collectors.collectingAndThen(Collectors.counting(), Long::intValue)));
  ...
  public List<String> getTopMovies(int top_k, String by) {
    if ("runtime".equals(by)) {
      return dataset.stream().filter(s -> s[4].length() != 0).sorted((s1, s2) -> {
        int c1 = Integer.parseInt(s2[4].substring(0, s2[4].length() - 3).trim())
            - Integer.parseInt(s1[4].substring(0, s1[4].length() - 3).trim());
    ...
    if ("rating".equals(by)) {
      return dataset.stream().filter(s -> s[6].length() != 0)
          ...
          .collect(Collectors.groupingBy(s -> s[1],
              Collectors.averagingDouble(s -> Float.parseFloat(s[0]))))
          .entrySet().stream()
          .sorted((e1, e2) -> {
      ...
      return dataset.stream().filter(s -> s[15].length() != 0)
          ...
          .collect(Collectors.groupingBy(s -> s[1],
              Collectors.averagingLong(s -> Long.parseLong(s[0].replace(",", "")))))
          .entrySet().stream()
          .sorted((e1, e2) -> {
            int c1 = e2.getValue().compareTo(e1.getValue());
            return c1 != 0 ? c1 : e1.getKey().compareTo(e2.getKey());
  ...
  public List<String> searchMovies(String genre, float min_rating, int max_runtime) {
    return dataset.stream()
        .filter(s -> s[6].length() != 0 && Float.parseFloat(s[6]) >= min_rating)
        .filter(s -> s[4].length() != 0
            && Integer.parseInt(s[4].substring(0, s[4].length() - 3).trim()) <= max_runtime)
